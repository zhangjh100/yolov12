...
class Mlp(nn.Module):
    def __init__(self, in_features, hidden_features=None, out_features=None):
        ...
        self.w = nn.Sequential(Conv(in_features, hidden_features, 1), Conv(hidden_features, out_features, 1, act=False))
...
class A2C2f(C2f):
    ...
    def __init__(self, c1, c2, n=1, a2=True, win=True, num_heads=2, drop_path=0.0, e=0.5, area=4, mlp_ratio=1.2, g=1, shortcut=True):
        ...
        self.c = int(c2 * e)  # hidden channels
        self.attn_c = round(self.c / 64) * 64

        self.cv1 = Conv(c1, 2 * self.c, 1, 1)
        # self.cv2 = Conv((2 + n) * self.c, c2, 1)  # optional act=FReLU(c2)
        self.cv2 = Conv(2 * self.c + n * self.attn_c, c2, 1)  # optional act=FReLU(c2)

        self.m = nn.ModuleList(
            AC3(self.attn_c, self.attn_c, 2, 1., num_heads, area, mlp_ratio, win, drop_path) if a2 else Bottleneck(self.attn_c, self.attn_c, shortcut, g) for _ in range(n)
        )
...
class AC3(nn.Module):
    ...
    # def __init__(self, c1, c2, n=1, e=0.5, num_heads=2, mlp_ratio=2.0, win=True, shortcut=True, g=1):
    def __init__(self, c1, c2, n=1, e=1., num_heads=2, area=4, mlp_ratio=1.5, win=True, drop_path=0.0):
        """Initializes the C3k module with specified channels, number of layers, and configurations."""
        super().__init__()
        c_ = int(c2 * e)  # hidden channels
        self.cv1 = Conv(c1, c_, 5, 1, 2, g=c_, act=False)
        # self.cv2 = Conv(2 * c_, c2, 1)  # optional act=FReLU(c2)
        self.m = nn.Sequential(*(ABlock(dim=c_, num_heads=num_heads, mlp_ratio=mlp_ratio, win=win, area=area, drop_path=drop_path) for i in range(n)))
